...
export default function LoginForm() {
  ...
  const getInputBorderColor = (focused: boolean, hovered: boolean) => {
    if (focused) return '#5865f2';
    if (hovered) return '#040405';
    return '#1e1f22';
  };
  ...
  const inputStyle = (focused: boolean, hovered: boolean) => ({
    width: '100%',
    height: '40px',
    padding: '10px',
    backgroundColor: '#1e1f22',
    border: `1px solid ${getInputBorderColor(focused, hovered)}`,
    borderRadius: '3px',
    color: '#dbdee1',
    fontSize: '16px',
    fontFamily: 'inherit',
      ...
      <div style={{
        ...
        <form onSubmit={handleSubmit(onSubmit)} style={{ textAlign: 'left' }}>
          ...
          <button
            ...
            onMouseDown={(e) => {
              (e.currentTarget as HTMLButtonElement).style.backgroundColor = '#3c45a5';
            }}
            onMouseUp={(e) => {
              (e.currentTarget as HTMLButtonElement).style.backgroundColor = '#4752c4';
            }}
